...
public class InitMongoDBDataChangeLog {
    ...
    public void initComments(MongoDatabase db, CommentRepository commentRepository, BookRepository bookRepository) {
        initComments(commentRepository, bookRepository, 0, "Не понравилось", "Книга вкусно пахнет");
        initComments(commentRepository, bookRepository, 1, "Много букв", "Понравилось");
        initComments(commentRepository, bookRepository, 2, "Интересно", "Захватывающе", "Закрученный сюжет");
    }

    private void initComments(CommentRepository commentRepository, BookRepository bookRepository,
                              int bookIndex, String... contents) {
        List<Comment> comments = new ArrayList<>();
        Book book = books.get(bookIndex);
        Stream.of(contents).forEach(content -> comments.add(commentRepository.save(new Comment(null, book, content))));
        comments.forEach(comment -> comment.setBook(null));
